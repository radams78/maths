...
\begin{cor}
    ...
    If $\BB$ is a basis for the topology $\TT$ then $\BB \subseteq \TT$.
\end{cor}
...
\begin{df}[Standard Topology on the Real Line]
    The \emph{standard topology on the real line} is the topology on $\RR$ generated by the basis consisting of all open intervals
    $(a,b)$.
\end{df}
...
\begin{proof}
    ...
    \begin{proof}
        \pf\ Take $(a,b) = (x-1,x+1)$.
    \end{proof}
    \step{2}{For any open intervals $(a,b)$, $(c,d)$ if $x \in (a,b) \cap (c,d)$, then there exists an open interval $(e,f)$ such that
