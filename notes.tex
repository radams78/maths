...
\begin{proof}
    \pf
    \step{1}{\pflet{$D$ be the metric on $\RR^\omega$ defined by $D(x,y) = \sup_i (\overline{d}(x_i, y_i) / i)$ where $\overline{d}$ is the standard bounded metric.}}
    \begin{proof}
        \step{a}{$D(x,y) \geq 0$}
        ...
        \begin{proof}
            \pf
            \begin{align*}
                D(x,z) & = \sup_i \frac{\overline{d}(x_i, z_i)}{i} \\
                & \leq \sup_i \frac{\overline{d}(x_i, y_i) + \overline{d}(y_i, z_i)}{i} \\
                & \leq \sup_i \frac{\overline{d}(x_i, y_i)}{i} + \sup_i \frac{\overline{d}(y_i, z_i)}{i} \\
                & = D(x,y) + D(y,z)
            \end{align*}
        \end{proof}
    \end{proof}
    ...
    \begin{proof}
        ...
        \step{c}{$B_D(a,\epsilon) = \prod_{i=1}^\infty U_i$ where $U_i = (a_i - i \epsilon, a_i + i \epsilon)$ if $i \leq N$, and $U_i = \RR$ if $i > N$}
    \end{proof}
    \step{3}{For any open set $U$ and $a \in U$, there exists $\epsilon > 0$ such that $B(a, \epsilon) \subseteq U$.}
    \begin{proof}
        \step{a}{\pflet{$n \geq 1$, $V$ be an open set in $\RR$ and $a \in \inv{\pi_n}(V)$}}
        \step{b}{\pick\ $\epsilon > 0$ such that $(a - \epsilon, a + \epsilon) \subseteq V$}
        \step{c}{$B(a, \epsilon / n) \subseteq \inv{\pi_n}(V)$}
    \end{proof}
    \qed
\end{proof}
